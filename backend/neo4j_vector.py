...
class Neo4jConnect():
    ...
    def query(
        ...
        logging.info(f"最终执行的查询命令是: {query}, 查询命令中对应的变量参数是: {params}")
        params = params or {}
...
class Neo4jVector(VectorStore):
    ...
    def similarity_search_with_score(
        self, query: str, k: int = 4, with_relation=False
        ...
        docs = self.similarity_search_with_score_by_vector(
            embedding=embedding, k=k, query=query, with_relation=with_relation
        )
        return docs
    ...
    def similarity_search_with_score_by_vector(
        ...
        if kwargs.get("with_relation",False):
            docs = [(self.get_relation(doc), score) for doc,score in docs]
        return docs
    def get_relation(self, doc, limit=20):
        """
        根据节点信息获取对应的关系
        Args:
            doc ():  对meta中的id进行查询，获取对应的关系
            limit: 搜索的关系数量
        Returns: doc
        """
        id = doc.metadata.get("id")
        if id:
            query = f'MATCH p=(n)-[r]-(m) WHERE n.id={id} RETURN p LIMIT {limit}'
            results = self.query(query)
            if results:
                doc.metadata["relation"] = results
        return doc
