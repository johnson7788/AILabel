  ...
  <div class="main">
    ...
    <div class="mb-6">
      <div v-for="(paragraph, index) in paragraphData" :key="index" class="mb-4">
        ...
        <p class="text-gray-600" v-html="highlightedSummary(paragraph.summary, paragraph.matches_answers)">
        </p>
      </div>
    </div>

    <!-- Sidebar -->
    <div class="sidebar fixed top-24 right-5 w-52 p-4 bg-gray-100 border border-gray-300 shadow-lg" v-if="hoveredData">
...
function highlightedSummary(summary: string, matches: Array<any>) {
  if (!matches) {
    return summary;
  }
  ...
  matches.forEach((match) => {
    ...
    highlightedText = highlightedText.replace(regex, (matchedWord) => {
      return `<span class="highlighted-word underline cursor-pointer text-blue-500" @mouseover="showSidebar(${JSON.stringify(match)})">${matchedWord}</span>`;
    });
  });
